...
function item(input){
 ...
 return {title:text(input.title,180,true),type,customerId:input.customerId?id(input.customerId):null,startMs:input.startMs,durationMinutes:duration,endMs:input.startMs+duration*60000,timeZone,
  location:text(input.location||'',400),assignedPeople:people(input.assignedPeople||[]),notes:text(input.notes||'',4000),status:choice(input.status||(type==='task'?'open':'scheduled'),type==='task'?['open','done','canceled']:['tentative','scheduled','in_progress','completed','canceled']),linkedItemId:input.linkedItemId?id(input.linkedItemId):null};
}
function conflicts(candidate,items,resolve=x=>x){
 ...
 return items.filter(x=>x.removedAtMs==null&&x.id!==candidate.id&&!['canceled','completed','done'].includes(x.status)&&x.startMs-(x.emailLink?.bufferMinutes||0)*60000<candidate.endMs&&x.endMs+(x.emailLink?.bufferMinutes||0)*60000>candidate.startMs)
...
function removalAction(row){
 const fields=['id','title','type','customerId','startMs','durationMinutes','endMs','timeZone','location','assignedPeople','notes','status','linkedItemId','estimate','businessId','version','createdAtMs','updatedAtMs','updatedBy','removedAtMs','removedBy','removalAction','emailLink'];
