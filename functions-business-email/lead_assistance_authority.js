...
const keys=['adaptiveOutreach','autonomyMode','replyMode','timeZone','expiresAt','termMode','ownerStopLocal','audiences','services','voice','claims','destinations','limits',
...
function cleanPolicy(value){
 ...
 const adaptiveError=require('./adaptive_outreach').validate(value);if(adaptiveError)fail('invalid-argument','Review the outreach objective and a distinct alternative message.');
...
function createAssistanceAuthority({db,now=Date.now,providerReady=false}){
 ...
 async function mutate(a,input){
  ...
  return db.runTransaction(async tx=>{
   ...
   const saved={modelAuthorizationPending:action==='prepare'?false:['activate','resume'].includes(action)?availableOnly:old?.modelAuthorizationPending===true,intakeStartsAt,authorizedIntake,businessId:a.businessId,agentType:'lead_generator',capability:'business_email',version,status,
