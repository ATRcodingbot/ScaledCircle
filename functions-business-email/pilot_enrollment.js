...
function createEnrollment({db,now=Date.now,workspaces=WORKSPACES,mailboxes=MAILBOXES}){
 ...
 async function recordDataReview(a,input){
  ...
  return db.runTransaction(async tx=>{const grant=(await tx.get(grantRef)).data(),old=(await tx.get(ref)).data();
   if(grant?.status==='active'&&(grant.dataReviewDigest||!grant.purposeReviewDigests?.outbound_business_context))fail('Do not change the model-data assessment during an active pilot.');
 ...
 async function activation(tx,a,saved,readiness){
  ...
  if(grant.status==='active'){if(!valid(grant,a.businessId,now(),workspaces)||(grant.dataReviewDigest&&grant.dataReviewDigest!==digest(review)))fail('The pilot authority expired or its reviewed configuration changed.');const own=(await tx.get(access(a.businessId))).data();if(own?.revokedAt||own?.expiresAt<=now())fail('The workspace pilot access expired.');if(saved.modelAuthorizationPending||saved.policy.modelDataConsent!==true||saved.approvedBy!==a.businessId)fail('Explicit current owner Gmail-model consent required.');return {activate:true,startsAt:grant.startsAt,expiresAt:Math.min(grant.expiresAt,own?.expiresAt||grant.expiresAt),apply(){if(!grant.dataReviewDigest){tx.update(grantRef,{dataReviewDigest:digest(review)});tx.create(grantRef.collection('audit').doc('gmail_purpose_bound'),{actorUid:a.actorUid,action:'gmail_review_bound_without_term_reset',at:now(),startsAt:grant.startsAt,expiresAt:grant.expiresAt,dataReviewDigest:digest(review)});}}};}
  ...
  const ready=policies.every((p,i)=>p?.modelAuthorizationPending!==true&&p?.policy?.modelAssistance===true&&p.policy.modelDataConsent===true&&p?.approvedBy===workspaces[i]&&['awaiting_pilot_activation','active'].includes(p.status)&&!p.revokedAt&&require('./assistance_term').resolveTerm(p.policy,grant,now()).expiresAt>now()&&
