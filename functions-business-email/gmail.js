...
function createProvider({clientId,clientSecret,redirectUri,fetchImpl=fetch}) {
  ...
  return {
    ...
    async history(refreshToken,{q,pageToken,maxResults=25}) {
      ...
      const access=(await token({grant_type:'refresh_token',refresh_token:refreshToken})).access_token;
      return request('https://gmail.googleapis.com/gmail/v1/users/me/threads?'+new URLSearchParams({q,maxResults:String(maxResults),includeSpamTrash:'false',...(pageToken?{pageToken}:{})}),{headers:headers(access)});
    },
  ...
  const matches=messages.filter(m=>m.id!==operation.providerMessageId&&m.threadId===operation.providerThreadId&&
    ...
    .map(m=>({providerMessageId:m.id,receivedAt:Number(m.internalDate),body:textBody(m.payload).slice(0,8000),
      classification:require('./lead_assistance_policy').classifyInbound({from:header(m,'from'),subject:header(m,'subject'),body:textBody(m.payload),
        headers:Object.fromEntries((m.payload?.headers||[]).filter(h=>['auto-submitted','x-autoreply','x-autorespond','list-id','list-unsubscribe','precedence','content-type','x-scaledcircle-notification','x-scaled-circle-notification'].includes(h.name.toLowerCase())).map(h=>[h.name,h.value]))}),
