...
exports.businessOperationsV1=onCall({region:'us-east1',maxInstances:4,timeoutSeconds:60},async request=>{
 ...
 try{return await createService({db,FieldValue,authority:createAuthority({db,auth:getAuth(app),FieldValue,Timestamp,project,
  internalAdminUid:process.env.GROWTH_PRODUCTION_ADMIN_UID||'',agentBusinesses:process.env.BUSINESS_OPERATIONS_AGENT_ALLOWED_BUSINESSES||''})}).execute(request);}
