...
function createService({db,FieldValue,authority,now=Date.now}){
 ...
 async function mutate(request,automaticAcceptance=null){
  ...
  return db.runTransaction(async tx=>{
   ...
   if(operation==='saveSchedulingAvailability'){
    ...
    if(input.emailConversation){
     if(!a.isOwner)m.fail('permission-denied','The Business owner must confirm this appointment workflow.');
     requirePermission(a,'communicationsRead');
     if(!['estimate','meeting'].includes(data.type)||!['tentative','scheduled'].includes(data.status)||input.newCustomer||input.overrideConflict)
      m.fail('invalid-argument','Choose a linked customer appointment without overriding conflicts.');
     const availability=(await tx.get(ref(a.businessId,'settings','scheduling'))).data();
     emailBinding=await require('./email_scheduling').binding({db,tx,businessId:a.businessId,input:input.emailConversation,item:data,current,availability,now:now(),automaticAcceptance});
    }else if(current?.emailLink&&['startMs','endMs','customerId','status'].some(k=>current[k]!==data[k])){
    ...
    if((!current||current.status==='tentative')&&data.status==='scheduled'&&customerBefore&&['estimate','meeting'].includes(data.type))await learningEvent(customerBefore,'appointment',itemId);
    ...
    if(customerBefore&&(!current||current.status==='tentative'||current.customerId!==data.customerId)&&['estimate','job','follow_up'].includes(data.type)&&data.status==='scheduled'&&can(a,'customersEdit')){const stage=data.type==='estimate'?'estimate_scheduled':data.type==='job'?'job_scheduled':'follow_up';queue(ref(a.businessId,'customers',data.customerId),{stage,version:customerBefore.version+1,updatedAtMs:now()},true);event(data.customerId,'stage_changed','Customer stage updated',{itemId,stage});}
    ...
    result={itemId,customerId:data.customerId,saved:true,conflictOverride:overlaps.length>0,...(emailBinding?.link.acceptanceCode?{acceptanceCode:emailBinding.link.acceptanceCode}:{}),confirmationEmailState:emailBinding?'not_requested':null};
    ...
    if(before.emailLink&&status==='scheduled'&&before.status!=='scheduled')m.fail('failed-precondition','Confirm the accepted appointment from its conversation.');
 ...
 async function acceptEmailOffer(request){
  const a=await authority(request,{write:true});if(!a.isOwner)m.fail('permission-denied','Owner-bound Schedule authority required.');
  const policy=(await db.doc(`agentPermissions/${a.businessId}_lead_generator/authorizations/business_email`).get()).data();
  if(policy?.status!=='active'||policy.revokedAt||policy.policy.expiresAt<=now()||policy.policy.bookingEnabled!==true)return {state:'not_authorized'};
  const offered=await root(a.businessId).collection('items').where('status','==','tentative').limit(101).get();
  if(offered.size>100)return {state:'bounded_review_required'};
  const results=[];
  for(const doc of offered.docs){
   ...
   const exact={};for(const key of ['title','type','customerId','startMs','durationMinutes','timeZone','assignedPeople','location','notes'])if(item[key]!==undefined)exact[key]=item[key];
   exact.status='scheduled';
   try{results.push(await mutate({auth:{uid:a.actorUid},data:{businessId:a.businessId,operation:'saveItem',requestId:'email_accept_'+m.hash([doc.id,item.version,latest.id]),input:{itemId:doc.id,expectedVersion:item.version,item:exact,
    emailConversation:{operationId:item.emailLink.operationId,inboundDigest:require('./email_scheduling').inboundDigest(rows),availabilityVersion:item.emailLink.availabilityVersion,acceptanceReplyId:latest.id}}}}, {code,version:item.version}));}
   catch(error){await db.runTransaction(async tx=>{const fresh=(await tx.get(doc.ref)).data();if(fresh?.version===item.version&&fresh.status==='tentative')tx.update(doc.ref,{'emailLink.lastAcceptanceMessageId':latest.id,'emailLink.acceptanceState':'needs_owner_review'});});results.push({itemId:doc.id,state:'needs_owner_review',reason:['aborted','failed-precondition'].includes(error.code)?'availability_or_conversation_changed':'authorization_check_required'});}
  }
  return {results};
 }
