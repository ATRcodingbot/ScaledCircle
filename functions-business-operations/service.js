 ...
 async function mutate(request,automaticAcceptance=null){
  ...
  return db.runTransaction(async tx=>{
   ...
   if(operation==='saveSchedulingAvailability'){
    ...
    if(!controlledSchedule&&customerBefore&&(!current||current.status==='tentative'||current.customerId!==data.customerId)&&['estimate','job','follow_up'].includes(data.type)&&data.status==='scheduled'&&can(a,'customersEdit')){const stage=data.type==='estimate'?'estimate_scheduled':data.type==='job'?'job_scheduled':'follow_up';queue(ref(a.businessId,'customers',data.customerId),{stage,version:customerBefore.version+1,updatedAtMs:now()},true);event(data.customerId,'stage_changed','Customer stage updated',{itemId,stage});}
