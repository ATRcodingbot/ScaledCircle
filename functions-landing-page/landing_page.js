...
const landingPageMedia = require("./landing_page_media");
...
function sanitizeDraft(input = {}) {
  ...
  return {headline, supportingText, valuePoints: points, ctaType,
    ...
    contactFields: [...new Set(contactFields)],
    media: landingPageMedia.sanitizeMediaSelection(input.media),
    privacyDisclosure: "By submitting, you agree that this Business may contact you about your request."};
}
...
function renderPage({page, version, formAction = "/landing-page-submit"}) {
  ...
  const media=version.renderMediaSnapshot||version.mediaSnapshot||null;
  const bucketName=text(version.mediaBucket,240);const imageUrl=(item)=>item&&bucketName?
    landingPageMedia.publicUrl(bucketName,item.publicDerivativePath):null;
  const srcset=(item)=>(item?.derivatives||[]).map((derivative)=>
    `${landingPageMedia.publicUrl(bucketName,derivative.publicDerivativePath)} ${Number(derivative.width)||960}w`).join(", ");
  const logoUrl=imageUrl(media?.logo);const heroItem=(media?.visuals||[]).find((item)=>item.role==="hero")||null;
  const heroUrl=imageUrl(heroItem);const serviceItems=(media?.visuals||[]).filter((item)=>item.role==="service");
  const disclosure=(item)=>String(item?.origin||"").startsWith("generated")?
    `<figcaption><strong>Conceptual service visual.</strong> ${escapeHtml(item.truthfulnessDisclosure||"Not a photo of this Business's completed work.")}</figcaption>`:"";
  const logo=logoUrl?`<img class="brand-logo" src="${escapeHtml(logoUrl)}" alt="${escapeHtml(media.logo.altText)}" width="${Number(media.logo.width)||960}" height="${Number(media.logo.height)||720}">`:"";
  const heroVisual=heroUrl?`<figure class="hero-visual"><img src="${escapeHtml(heroUrl)}" srcset="${escapeHtml(srcset(heroItem))}" sizes="(max-width:820px) calc(100vw - 32px), 42vw" alt="${escapeHtml(heroItem.altText)}" width="${Number(heroItem.width)||1920}" height="${Number(heroItem.height)||1080}" fetchpriority="high">${disclosure(heroItem)}</figure>`:`<aside class="hero-panel" aria-label="What to expect"><strong>A straightforward way to get started</strong><span>Describe the service you’re looking for</span><span>Choose email or phone for a reply</span><span>Continue directly with the Business</span></aside>`;
  const serviceVisuals=serviceItems.length?`<section class="section service-visuals" aria-label="Service visuals"><div class="shell service-visual-grid">${serviceItems.map((item)=>`<figure><img src="${escapeHtml(imageUrl(item))}" srcset="${escapeHtml(srcset(item))}" sizes="(max-width:820px) calc(100vw - 32px), 33vw" alt="${escapeHtml(item.altText)}" width="${Number(item.width)||960}" height="${Number(item.height)||720}" loading="lazy">${disclosure(item)}</figure>`).join("")}</div></section>`:"";
  const brand=media?.useBrandColors&&media.brand?media.brand:null;
  const brandVars=brand?`--accent:${escapeHtml(brand.primaryColor||"#176fd1")};--accent-dark:${escapeHtml(brand.secondaryColor||brand.primaryColor||"#0d4c99")};`:"";
...
:root{color-scheme:light;--ink:#10243e;--muted:#526579;--accent:#176fd1;--accent-dark:#0d4c99;--soft:#edf5fc;--surface:#fff;--line:#d7e2ec;--hero:#e9f4ff;--radius:22px;--shadow:0 18px 55px rgba(16,36,62,.12);--display:system-ui,sans-serif;--body:system-ui,sans-serif;${brandVars}}
...
function createLandingPageService({db, FieldValue, now = () => Date.now(), randomBytes = crypto.randomBytes,
  ...
  async function transition(input, actor) {
    ...
    if(action==="publish"){
      ...
      if(!pageSnap.exists||pageSnap.data().businessUid!==actor?.uid)throw new Error("landing_page_forbidden");
      const page=pageSnap.data();const versionRef=pageRef.collection("versions").doc(page.draftVersionId);
      const versionSnap=await versionRef.get();if(!versionSnap.exists)throw new Error("landing_page_version_missing");
      if(page.status==="published"&&page.publishedVersionId===page.draftVersionId&&
          versionSnap.data().mediaPublication?.status==="complete"){
        return{status:"published",publicSlug:page.publicSlug,versionId:page.draftVersionId,
          responseAssetId:page.responseAssetId||null,trackedUrl:null,idempotentReplay:true};
      }
      const selection=versionSnap.data().content?.media||{};
      const mediaSnapshot=await landingPageMedia.materializeSelection({db,bucket,businessUid:page.businessUid,
        pageId:pageRef.id,versionId:page.draftVersionId,selection});
      const activeBucket=typeof bucket==="function"?bucket():bucket;
      prepared={draftVersionId:page.draftVersionId,contentDigest:versionSnap.data().contentDigest,
        mediaSnapshot,mediaBucket:activeBucket?.name||null};
    }
    await db.runTransaction(async(tx)=>{
      ...
      const p=snap.data();
      if(action==="publish"){
        if(!prepared||p.draftVersionId!==prepared.draftVersionId)throw new Error("landing_page_publish_state_changed");
        ...
        if(v.data().contentDigest!==prepared.contentDigest)throw new Error("landing_page_publish_state_changed");
        ...
        tx.set(versionRef,{submissionContext,publishedAt,published:true,mediaSnapshot:prepared.mediaSnapshot,
          mediaBucket:prepared.mediaBucket,mediaPublication:{status:"complete",completedAt:FieldValue.serverTimestamp()}},{merge:true});
  ...
  async function resolve(publicSlug) {
    ...
    const version={id:v.id,...v.data()};
    if(version.mediaSnapshot){version.renderMediaSnapshot=await landingPageMedia.availableSnapshot({bucket,snapshot:version.mediaSnapshot});}
    return {page,version};
  }
