...
function createService({db,FieldValue,project,target,readSource=fetchSource,now=Date.now,areaPriorityIds=[],sourceCatalog=sources,customerContext=null}) {
  ...
  async function run() {
    ...
    const selected=geography.prioritizeSources(sourceCatalog,scope,existing.map(p=>p.sourceUrl)).slice(0,12);
    const results=[];
  ...
  async function makeReport(kind) {
    ...
    const id='growth_report_'+hash([target,kind,period,...(registered?[scope.preferenceVersion]:[])]).slice(0,40),summary={...report(rows,runs.filter(r=>r.schemaVersion===VERSION),scope,observations,sourceCatalog),newApprovalsToday:rows.filter(r=>r.qualified&&day(r.discoveredAt)===day(now())).length};
