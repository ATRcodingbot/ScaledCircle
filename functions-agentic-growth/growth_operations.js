...
const publicHost=value=>{try{return new URL(value).hostname.replace(/^www\./,'');}catch{return null;}};
...
function report(rows,runs,scope={status:'MISSING_MAINTAINED_GEOGRAPHY',areas:[]},observations=[],sourceCatalog=sources,outreach=null) {
  ...
  const discovery=require('./research_result').freshness(rows,runs);
  return {discovery,serviceAreaStatus:scope.status,serviceAreaPriority:scope.areas.map(a=>a.label),discoveryByServiceArea:groups,businessesFound:count('business'),partnersFound:count('referral_partner'),individualScalersFound:count('scaler'),qualified:rows.filter(x=>x.qualified).length,awaitingApproval:rows.filter(x=>x.approvalState==='awaiting_approval').length,
...
function createService({db,FieldValue,project,target,readSource=fetchSource,now=Date.now,areaPriorityIds=[],sourceCatalog=sources,customerContext=null,publicResearch=null}) {
  ...
  async function run() {
    ...
    // Transport and central research budget are injected only after a separately
    // authorized pilot. No key, paid client or Social allowance is used here.
    if(publicResearch){
    ...
    const suppressedCount=[...discovered.sources,...sourceCatalog].filter(source=>suppressedRecipients.has(source.email?.toLowerCase())).length;
    ...
    await db.runTransaction(async tx=>{
      ...
      for(const result of records){const {source,observation:o,id,old}=result,agentType=source.kind==='business'?'lead_generation':'workforce_recruiter';
        ...
        if(source.publicDiscovery)Object.assign(prospect,{publicDiscovery:source.publicDiscovery,explicitNeed:false,unknowns:source.unknowns});
      ...
      tx.update(ref,{status:'completed',...((customerContext?.researchVersion||publicResearch)?{discoveryVersion:customerContext?.researchVersion||'PublicWebDiscoveryV1',discoveryChecks:discovered.checks}:{}),serviceAreaStatus:scope.status,serviceAreaPriority:scope.areas.map(a=>a.label),geographyPreferenceVersion:scope.preferenceVersion,newQualifiedProspectCount:records.filter(r=>r.observation?.qualified&&!r.old.exists).length,existingProspectsRechecked:records.filter(r=>r.old.exists).length,suppressedCount,newProspectCount:records.filter(r=>r.observation&&!r.old.exists).length,duplicatesExcludedCount:duplicatesExcludedCount+records.filter(r=>r.observation&&r.old.exists).length,sourceChecks:results.filter(r=>r.observation).length,unavailableSources:results.filter(r=>r.error).length,completedAt:now(),result:discovered.checks.some(c=>c.attemptId&&c.status!=='search_completed')?'Fresh discovery failed or is partial; catalog checks are not new discovery. External contact remains held.':'Research processing complete; inspect qualification and new-record counts. External contact remains held.',leaseUntil:0});
    ...
    await makeReport('daily');await makeReport('weekly');return {runId,reused:false,sourceChecks:results.filter(r=>r.observation).length,discoveryOutcome:require('./research_result').cycle({discoveryChecks:discovered.checks})};
  }
  ...
  async function load() {
    ...
    const schedule=customer?(await db.doc('customerResearchSchedules/'+target).get()).data():null;
    const nextEligible=customer?schedule?.nextRunAt:health.data()?.nextResearchAfter;
    summary.next=require('./research_result').describe(summary.discovery,nextEligible);
