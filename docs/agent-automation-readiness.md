...
The exact Batch 4 candidate is now **PRODUCTION REVIEWED — NOT DEPLOYED**. The coordinated manifest is `docs/p0-batch-4-production-promotion-manifest.md`. Future release agents must preserve the distinction between staging verification, production review, deployment approval, and production verification. They may not infer deployment authority from a green review, deploy `completion-core:startTrackingSession`, accept agreements for users, relax the application Rule, access Stripe secrets, or perform payment/worker/Wallet actions. Professional attorney review and the physical Android worker-lifecycle gate remain open.
