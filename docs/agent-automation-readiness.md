...
P0 Batch 3 replaces the conceptual Sales rows above with a first deterministic boundary: `getSalesPipeline`, `mutateSalesLead`, and `recordSalesActivity`. These human-operated, Admin-authorized launch APIs are live and production-verified with zero secrets. The staging Internal QA flow proved structured suppression and activity/follow-up state without outbound delivery; production access and denial checks passed without copying the fixture. Future agents may consume the redacted output, suppression state, follow-up schedule, attribution, and activity history, but autonomous discovery/research/outreach remains **NOT YET IMPLEMENTED**.

Attribution Foundation V1 proposes the next deterministic read boundary: `getAttributionOverview` exposes bounded response assets, interactions, explicit lead milestones, and known conversions. Future agents may consume that supported API after staging/production certification; they may not scrape UI, follow opaque redirects to infer private tenant data, or write attribution collections. Autonomous opportunity generation, outreach, provider integration, and economic mutation remain **NOT YET IMPLEMENTED**.

P0 Batch 4 defines a zero-secret `legal-core` boundary with fixed agreement versions, authenticated own-user scope, authoritative role context, server timestamps, deterministic replay, and bounded own-user status. Owning server actions independently fail closed with structured missing-version data; future agents may observe that status but may not accept for users, invent policy, change legal copy, infer worker classification, or treat rendered UI as authority. Staging-hosted QA verified that direct application writes remain Rules-denied, `application-core` creates one post-consent application, and assignment authority blocks a legacy applicant until current work consent. Material legal decisions remain Class C and require human/counsel approval. Location-notice enforcement remains deferred to the worker-lifecycle production gate.

The exact Batch 4 consent-engineering scope is now **PRODUCTION DEPLOYED + VERIFIED**. The coordinated evidence is in `docs/p0-batch-4-production-promotion-manifest.md`. Future agents may read structured consent status but may not accept agreements for users, relax the application Rule, access Stripe secrets, or perform payment/worker/Wallet actions. `completion-core:startTrackingSession` location enforcement is still deferred; professional attorney review and the physical Android worker-lifecycle gate remain open.
