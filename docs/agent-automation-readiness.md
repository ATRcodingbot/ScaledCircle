...
P0 Batch 4 adds a staging-only, zero-secret `legal-core:recordLegalConsent` boundary with fixed agreement versions, authenticated own-user scope, authoritative role context, server timestamps, and deterministic replay behavior. Future agents may read approved agreement-version status through a purpose-built interface, but may not accept agreements for users, invent policy, change legal copy, infer worker classification, or treat rendered UI as consent authority. Material legal decisions remain Class C and require human/counsel approval.
