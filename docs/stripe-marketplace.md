...
`exports.stripeWebhook` in `functions/index.js` is the single canonical Stripe webhook. The HTTP webhook verifies the Stripe signature against the raw request body. `stripeEvents` records move through received/processing/processed/failure states, making duplicate delivery harmless. Redirect URLs never confirm payment. Handlers reconcile current Stripe objects when ordering matters and reject amount, currency, Customer, Business, or campaign mismatches.
