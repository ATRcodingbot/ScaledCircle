...
- Gmail Read retrieves explicitly authorized new-inquiry label threads after owner authorization and already-linked conversations independently of labels. A one-time owner-created Gmail filter automatically applies the selected label to future matching mail. ScaledCircle does not request Gmail settings scopes or silently search the whole inbox. The filter coverage is owner-confirmed, not provider-inspected.
