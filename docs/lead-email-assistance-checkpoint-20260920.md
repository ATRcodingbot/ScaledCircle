...
8. Pure bounded Email policy preflight/recipient/dispatch/reply-approval checks are prepared with fixture tests. They are **not yet a persisted opt-in or recurring automatic-send implementation**.

## Remaining execution work (do not claim complete)

- Persist one owner-reviewed pilot playbook/permission through maintained `bounded_managed` authority and explicit exact-workspace enrollment, with independent intro/follow-up/reply/booking settings; pause/revoke; exact sender generation; immutable audit and term. Do not flip the legacy research external-actions flag.
