...
| Admin | Open issues | admin dashboard | `adminIssues` | PARTIAL | FIX BEFORE LAUNCH | P0 | Rules/backend | Required | Web | Indirect | Not a unified operations queue | Build P0-7 minimum |
| Admin | Payments/refunds operations | none | Stripe/payment authorities | NOT IMPLEMENTED | FIX BEFORE LAUNCH | P0 | Backend only | Required | Web | Critical | Console dependency | Read-only correlation + approved actions |
| Admin | Completion/earning operations | none | completion/earning records | NOT IMPLEMENTED | FIX BEFORE LAUNCH | P0 | Backend only | Required | Web | Critical | Console dependency | Exception queue |
...
| Operations | Payment/refund monitoring | no unified screen | campaign-funding/events | PARTIAL | FIX BEFORE LAUNCH | P0 | Backend | Required | Web | Critical | Stripe/Firebase/log dependency | Correlated exception view |
| Operations | Email failure queue | no unified screen | email job collections | PARTIAL | FIX BEFORE LAUNCH | P1 | Backend | Required | Web | Indirect | Manual inspection | Retry/status API |
