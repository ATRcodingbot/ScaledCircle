...
| Public | Legal/consent surface | signup/footer/action prompts/tracking disclosure | `legal-core` + consent-gated funding/application/assignment live; tracking gate deferred | READY (ENGINEERING) | ATTORNEY REVIEW BEFORE BROAD LAUNCH | P0 | 334 backend; 292 Flutter + 1 skip; Firestore 20/20; Storage 6/6 | Staging actor QA and production exact-scope deployment verified | Web/mobile | High | High | Consent engineering live; location gate deferred; attorney review remains | Retain professional review and worker-lifecycle gates |
