...
| Admin | Secure admin login/gate | `/admin/login`, `/admin` | Auth + admin role | READY | LAUNCH | P0 | Backend/Flutter/hosted QA | Passed staging | Web | No | Critical | None material | Retain authoritative role/profile gate |
...
| Admin | Open issues | Admin Command Center | `admin-ops-core:getAdminOperationsOverview` | READY | LAUNCH | P0 | Backend/Flutter | Passed staging | Responsive web | Indirect | Production promotion pending | Promote reviewed P0 Batch 2 scope |
| Admin | Payments/refunds operations | Admin Command Center/timeline | reconciled payment/refund records via `admin-ops-core` | READY | LAUNCH | P0 | Backend/Flutter | Passed staging | Web | Critical | Read-only visibility; recovery remains purpose-built | Preserve financial authority separation |
| Admin | Completion/earning operations | Admin Command Center/timeline | completion/earning/Wallet read model via `admin-ops-core` | READY | LAUNCH | P0 | Backend/Flutter | Passed staging | Web | Critical | Production worker authority migration remains P0-2 | Keep read model compatible with current authority |
...
| Sales | Minimum lead funnel | disabled Admin card | none | NOT IMPLEMENTED | FIX BEFORE LAUNCH | P0 | None | Required | Web | Indirect | No usable workflow | Implement minimal internal funnel |
| Operations | Payment/refund monitoring | Admin Command Center/timeline | bounded reconciled payment/event read model | READY | LAUNCH | P0 | Backend/Flutter | Passed staging | Web | Critical | Typed recovery commands intentionally separate | Promote reviewed read model |
| Operations | Email failure queue | Admin Command Center | durable email job read model | READY | LAUNCH | P1 | Backend/Flutter | Passed staging zero-state/tests | Web | Indirect | Provider retry execution remains existing authority | Retain failed/exhausted/long-pending classifications |
| Operations | Provider outage handling | Admin Command Center health | provider errors/adminIssues + partial-load state | READY | LAUNCH | P1 | Backend/Flutter | Passed staging | Web | Indirect | Categorical status, not synthetic uptime | Add measured telemetry later |
| Operations | Support case handling | Admin Command Center | support cases + typed audited status transition | READY | LAUNCH | P0 | Backend/Flutter | Passed staging zero-state/tests | Web | Indirect | No full ticketing suite by design | Retain Open → In Progress → Resolved scope |
