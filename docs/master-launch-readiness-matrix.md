...
| Public | Legal/consent surface | signup/footer/action prompts/tracking disclosure | `legal-core` + consent-gated funding/application/assignment/tracking candidate | PARTIAL | FIX BEFORE LAUNCH | P0 | 334 backend; 292 Flutter + 1 skip; Firestore 20/20; Storage 6/6 | Public surface passed staging; action candidate local-only | Web/mobile | High | High | Required server-only application Rule is tested but undeployed; attorney review remains | Authorize/deploy exact Rule + Functions + Hosting, then hosted actor QA |
