...
function createPublisher({db,project,credentials,fetchImpl,now=Date.now,providerCreatesEnabled=false,enabledProviders=["facebook","instagram"],customerUids=[],actorAuth}) {
 ...
 async function context(tx,job,action) {
  ...
  if(customer) {
   ...
   if(!require("./subscription_entitlements").hasActiveManagedGrowthEntitlement(subscription)&&!await require('./social_internal_managed').authority({db,uid:job.businessUid,read}))throw Error('meta_customer_entitlement_required');
