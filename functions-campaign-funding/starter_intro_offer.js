...
function createService({db,FieldValue,workspace,legal,stripe,environment,projectId,priceId,ensureCustomer,now=Date.now}){
 ...
 async function finishCheckout(q,selected){const uid=q.businessId,claimId=q.claimId,attemptId=q.attemptId,ref=db.doc(CLAIMS+'/'+claimId);
    ...
    const s=await stripe.checkout.sessions.create({mode:'subscription',customer:q.customerId,line_items:[{price:priceId,quantity:1}],discounts:[{coupon:q.couponId}],allow_promotion_codes:false,payment_method_collection:'always',payment_method_types:['card'],automatic_tax:{enabled:false},expires_at:q.checkoutExpiresAtSeconds||Math.floor(now()/1000)+31*60,success_url:'https://scaledcircle.com/?billing=success',cancel_url:'https://scaledcircle.com/?billing=cancelled',metadata,subscription_data:{metadata:{...metadata,checkoutRequestId:attemptId}}},{idempotencyKey:'starter_intro_checkout_'+attemptId});
 ...
 async function resumeExpandedCoupon(q,selected,g){
  need(g.rollout==='pilot'&&g.resumeCouponExpansionClaims?.includes(q.claimId)&&q.status==='hold'&&!q.checkoutSessionId&&q.customerId&&q.renewalPreviewAtMs,'This purchase requires operator reconciliation.');
  ...
  await db.runTransaction(async tx=>{const [prior,w,e,cfg]=await Promise.all([tx.get(ref),tx.get(db.doc('wallets/'+q.businessId)),tx.get(db.doc('businessSubscriptions/'+q.businessId)),tx.get(db.doc(CONFIG))]);
   need(prior.data()?.status==='hold'&&!prior.data()?.recoveryStartedAt&&prior.data()?.attemptId===q.attemptId&&!prior.data()?.checkoutSessionId);
   need(permitted(cfg.data(),q.businessId,now())&&cfg.data().rollout==='pilot'&&cfg.data().resumeCouponExpansionClaims?.includes(q.claimId));
