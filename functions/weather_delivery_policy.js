...
function event(feature,now=Date.now()){
 ...
 const canonical=id=>String(id).replace(/^https:\/\/api\.weather\.gov\/alerts\//,'');
 const references=(p.references||[]).map(r=>r.identifier).filter(Boolean);
 const eventId=hash(canonical(references[0]||feature.id));
...
function decision({userId,job,preferences,entitled,owner,event:current,matches,now=Date.now()}){
 ...
 if(!entitled||preferences?.emailEnabled!==true||!Number.isFinite(preferences.emailEnabledAt))return {state:'suppressed',reason:'weather_email_disabled'};
 ...
 if(current.status==='expired'||(current.status!=='cancelled'&&current.expiresAt<=now))return {state:'suppressed',reason:'weather_event_expired'};
