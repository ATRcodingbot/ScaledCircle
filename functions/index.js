...
exports.provisionCampaignTracking = onCall(
  ...
  async (request) => {
    ...
    const requestedChannels = Array.isArray(request.data?.channels)
      ...
      : ["web", "qr", "print"];
    if (requestedChannels.some(channel => ['phone', 'email'].includes(channel))) {
      throw new HttpsError('failed-precondition', 'Phone and email tracking are not available yet. Choose website or QR tracking.');
    }
  ...
  const completions = completionSnapshots.docs
    ...
    .map((completion) => ({
      ...
      earning: earning && earning.scalerId === completion.scalerId ? {
        ...
        ...require('./job_room_approved_amounts').read({earning, zone, completion, zoneId}),
...
exports.getTrackingSessionState = trackingCallable(
  "getTrackingSessionState", async (request) => {
    ...
    if (request.data?.includeProgress === true) {
      ...
      try {
        const campaign = (process.env.GCLOUD_PROJECT || process.env.GOOGLE_CLOUD_PROJECT) === 'scaledcircle-staging'
          ? await db.collection('campaigns').doc(session.campaignId).get() : null;
        if (canvassingCompletion.applies(process.env.GCLOUD_PROJECT || process.env.GOOGLE_CLOUD_PROJECT,campaign?.data() || {})) {
          const accepted=routeProgress.acceptedEvidence({...session,sessionId},chunks.docs.map(d=>d.data()));
          progress=accepted?{...canvassingCompletion.coverage(zone.data(),accepted),provisional:true,
            path:accepted.map(p=>({latitude:p.latitude,longitude:p.longitude}))}:
            {state:'calculating',coveragePercentage:null,provisional:true};
        } else {
          progress = routeProgress.projectProgress({...session, sessionId}, zone.data(),chunks.docs.map(doc=>doc.data()),calculateRouteCompletion);
        }
      } catch (_) {
