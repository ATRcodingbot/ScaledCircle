...
before(async()=>{for(const edition of ['staging','production']){
 ...
 await env.withSecurityRulesDisabled(async c=>{const db=c.firestore();for(const uid of ['owner','other','admin'])await setDoc(doc(db,'users/'+uid),{role:uid==='admin'?'admin':'scaler',active:true});await setDoc(doc(db,'notifications/notice'),{userId:'owner',read:false});for(const col of ['mobilePushDevices','mobilePushTokenBindings','mobilePushReceipts','mobileNotificationPreferences','mobileNotificationSignalReceipts'])await setDoc(doc(db,col+'/record'),{uid:'owner',token:'never-client-visible'});});environments.push(env);
}});
after(async()=>Promise.all(environments.map(e=>e.cleanup())));
test('both deployed rule models deny every client token/receipt/preference read or write',async()=>{for(const e of environments){for(const uid of ['owner','other','admin']){const db=e.authenticatedContext(uid,{email_verified:true}).firestore();for(const col of ['mobilePushDevices','mobilePushTokenBindings','mobilePushReceipts','mobileNotificationPreferences','mobileNotificationSignalReceipts']){await assertFails(getDoc(doc(db,col+'/record')));await assertFails(setDoc(doc(db,col+'/forged'),{uid,token:'forged'}));}}}});
...
test('Admin has no implicit private-recipient detail or read receipt authority',async()=>{for(const e of environments){const admin=e.authenticatedContext('admin',{email_verified:true}).firestore();await assertFails(getDoc(doc(admin,'notifications/notice')));await assertFails(updateDoc(doc(admin,'notifications/notice'),{read:true}));}});
