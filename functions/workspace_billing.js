...
function createBillingService({db,FieldValue,workspace,stripe,planForPrice,priceForPlan,sync,validateProvider,validatePrice,now=Date.now}) {
 ...
 return {
  ...
  async preview({uid,businessId,plan,selection}) {
   ...
   await require('./product_availability').assertPurchase({db,businessId:a.businessId,selection:{plan},existingItems:[view.plan],now:now()});
  ...
  async change({uid,businessId,action,requestId,plan,quoteId}) {
   ...
   await db.runTransaction(async tx=>{
    ...
    if(action==='changePlan') {
      ...
      await require('./product_availability').assertPurchase({db,businessId:a.businessId,selection:{plan},existingItems:[view.plan],transaction:tx,now:now()});
