...
function createService({db,auth,FieldValue,project,appEnv,now=Date.now}) {
  ...
  function runtime() {
    if(project!=='scaledcircle-staging'||appEnv!=='staging')fail('failed-precondition','Account deletion is not enabled in this environment.');
  }
  async function inventory(uid,tx) {
    ...
    const queries=[['zones',db.collection('campaignZones').where('assignedScalerId','==',uid)],
    ...
    if(data.businessWorkspaces || data.businessSubscriptions || (u.role==='business'&&u.signupPurpose!=='team_invitation'))blockers.push('owner');
  ...
  async function close(uid,{confirmation,authTime}={}) {
    ...
    await db.runTransaction(async tx=>{
      ...
      tx.create(ref,{version:VERSION,uid,status:'closing',requestedBy:uid,authTime,confirmedAt:stamp(),
        ...
        previousRole:i.data.users.role,environment:'staging'});
