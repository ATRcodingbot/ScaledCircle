...
test('catalog failure resumes exact source once under concurrent visits and preserves Facebook bytes',async()=>{
 ...
 let calls=0,requestId;const media={prepareCandidate:async(owner,input,recommendation)=>{assert.equal(owner,uid);if(requestId)assert.equal(recommendation.requestId,requestId);requestId=recommendation.requestId;calls++;if(calls===1){const error=Error('catalog unavailable');error.preparationStage='model_catalog';throw error;}return {sha256:'same-existing-source',status:'blocked',subjectCheck:{status:'blocked',reasons:['Quality is genuinely below threshold']}};}};
 ...
 const results=await Promise.all([prep.prepare(uid,input),prep.prepare(uid,input)]);
