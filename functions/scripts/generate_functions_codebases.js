...
const creativeMediaExports = new Set([
...
const businessProfileExports = new Set(["getBusinessOnboarding", "saveBusinessOnboarding", "searchBusinessProfilePlaces", "getBusinessServiceAreaSuggestions", "saveBusinessGrowthProfile", "prepareInvitedBusinessAccount", "getBusinessTeam", "inviteBusinessTeamMember", "acceptBusinessTeamInvitation", "updateBusinessTeamMember", "getBusinessWorkspaceContext", "selectBusinessWorkspace", "listBusinessWorkspaceRecordIds", "auditBusinessCampaignDraft"]);
...
function transformIndex(mode) {
  ...
  if (!["attribution", "physical-marketing"].includes(mode)) {
    ...
    ast.program.body = ast.program.body.flatMap((statement) => {
      ...
      const declarations = statement.declarations.filter((declaration) =>
        !attributionHelpers.has(declaration.id?.name));
      return declarations.length ? [{...statement, declarations}] : [];
    });
  }
  if (mode !== "creative-media") {
    ...
    ast.program.body = ast.program.body.flatMap((statement) => {
      ...
      if (statement.type !== "VariableDeclaration") return [statement];
      const declarations = statement.declarations.filter((declaration) =>
        !creativeMediaHelpers.has(declaration.id?.name));
      return declarations.length ? [{...statement, declarations}] : [];
    });
  }
  if (mode !== "physical-marketing") {
    const physicalMarketingHelpers = new Set([
      "postcardFulfillment", "POSTCARD_STRIPE_TEST_KEY", "postcardService", "postcardCall",
  ...
  ast.program.body = ast.program.body.flatMap((statement) => {
    ...
    const declarations = statement.declarations.filter((declaration) => {
      ...
      if (mode === "physical-marketing") return identifier === "POSTCARD_STRIPE_TEST_KEY";
