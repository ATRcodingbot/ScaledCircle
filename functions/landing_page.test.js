...
test("public input cannot select lead owner or Business email recipient",async()=>{const db=memoryDb(landingSeed());const service=lp.createLandingPageService({db,FieldValue:{serverTimestamp:()=>"SERVER_TIME"},getAuthUser:async()=>({email:"verified-owner@example.invalid",emailVerified:true})});const result=await service.submit({slug:"PUBLIC_A",version:"version-a",name:"Public",email:"visitor@example.invalid",ownerUid:"attacker",businessUid:"attacker",recipient:"attacker@example.invalid",idempotencyKey:"authority"});assert.equal(db.docs.get(`salesLeads/${result.leadId}`).ownerUid,"business-a");const businessJob=[...db.docs.entries()].find(([path])=>path.startsWith("outboundEmailJobs/landing-business_"))[1];assert.equal(businessJob.to,"verified-owner@example.invalid");assert.match(businessJob.payload.inquiryUrl,/#\/business\/landing-pages\?pageId=page-a$/);});

test("new page creation is replay-idempotent for one Business request",async()=>{const db=memoryDb();const service=lp.createLandingPageService({db,FieldValue:{serverTimestamp:()=>"SERVER_TIME"}});const actor={uid:"business-a",role:"business"};const input={creationRequestId:"create-request-a",content:{headline:"Page A",supportingText:"A useful next step",contactFields:["name","email"]}};const first=await service.createDraft(input,actor);const replay=await service.createDraft(input,actor);assert.equal(replay.pageId,first.pageId);assert.equal(replay.versionId,first.versionId);assert.equal(replay.publicSlug,first.publicSlug);assert.equal(replay.idempotentReplay,true);assert.equal([...db.docs.keys()].filter((path)=>/^landingPages\/[^/]+$/.test(path)).length,1);assert.equal([...db.docs.keys()].filter((path)=>/\/versions\//.test(path)).length,1);});
