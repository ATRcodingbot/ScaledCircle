...
test('central response through evidence, qualification and CRM keeps processing failure distinct and restart duplicate-safe',async()=>{
 ...
 for(const status of ['research_response_invalid','research_response_incomplete']){
  const run=await research.run(),saved=(await db.doc('agentRuns/'+run.runId).get()).data();assert(saved.discoveryChecks.every(c=>c.status===status&&c.stage==='response_parsing'));assert.equal(saved.newProspectCount,0);assert.equal((await db.collection('agentCrmProspects').get()).size,0);const view=await research.load();assert.equal(view.summary.discovery.state,'failed');assert.equal(view.summary.discovery.lastSuccessfulProcessingAt,null);assert.match(view.summary.next,/response_parsing/);assert(view.notifications.some(n=>n.title==='Research needs attention'));
