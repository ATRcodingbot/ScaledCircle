...
test('central response through evidence, qualification and CRM keeps processing failure distinct and restart duplicate-safe',async()=>{
 ...
 clock+=86400000;item.url='https://example.com/new-vendor-page';const newPage=await research.run(),newPageRun=(await db.doc('agentRuns/'+newPage.runId).get()).data();assert.equal(newPageRun.newProspectCount,0);assert.equal(newPageRun.existingProspectsRechecked,1);assert.equal((await db.collection('agentCrmProspects').get()).size,1);const unchanged=(await db.collection('agentProspects').get()).docs[0].data();assert.equal(unchanged.discoveredAt,prospect.discoveredAt);assert.equal(unchanged.sourceUrl,prospect.sourceUrl);assert((await db.collection('agentObservations').where('runId','==',newPage.runId).get()).docs.some(d=>d.data().sourceUrl===item.url));
