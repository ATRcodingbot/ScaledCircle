...
test("Social Operations has one dedicated narrowly-secret-bound codebase", () => {
  ...
  assert.doesNotMatch(indexSource, /META_SOCIAL_APP_SECRET|X_SOCIAL_CLIENT_SECRET/);
});
...
test("YouTube OAuth codebase binds only shared encryption and the YouTube secret", () => {
  ...
  assert.doesNotMatch(indexSource, /META_SOCIAL_APP_SECRET|X_SOCIAL_CLIENT_SECRET/);
  assert.doesNotMatch(indexSource, /socialOAuthMetaCallbackV1|socialOAuthXCallbackV1/);
  assert.doesNotMatch(indexSource,
    /syncMetaSocialReadOnlyPerformanceV1|syncXSocialReadOnlyPerformanceV1/);
});
