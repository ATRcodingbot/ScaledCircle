...
void main() {
  test('pending Scaler setup reuses the existing preference screen and callables', () {
    final pending = File('lib/screens/public/early_access_pending_screen.dart').readAsStringSync();
    final service = File('lib/services/discovery_preferences_service.dart').readAsStringSync();
    ...
    expect(service, contains("httpsCallable('savePendingScalerPreferences')"));
  });

  test('Scaler work types are loaded from the server taxonomy, not a Flutter list', () {
    final screen = File('lib/screens/preferences/areas_preferences_screen.dart').readAsStringSync();
    final service = File('lib/services/discovery_preferences_service.dart').readAsStringSync();
  ...
  test('registration passes authoritative role into pending access', () {
    final register = File('lib/screens/auth/register_screen.dart').readAsStringSync();
