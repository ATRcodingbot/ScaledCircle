...
class _VerifyEmailScreenState extends State<VerifyEmailScreen> {
  ...
  Future<void> _verify() async {
    ...
    try {
      if (code.isNotEmpty) {
        await FirebaseAuth.instance.checkActionCode(code).timeout(const Duration(seconds: 20));
        await FirebaseAuth.instance.applyActionCode(code).timeout(const Duration(seconds: 20));
      }
      final user = await refreshIdentity();
      if (mounted) {
        setState(
          () => _state = code.isNotEmpty
              ? VerificationViewState.success
              : user?.emailVerified == true
              ? VerificationViewState.alreadyVerified
              : VerificationViewState.invalid,
        );
      }
      ...
      try {
        ...
        if (mounted) setState(() => _state = VerificationViewState.invalid);
        return;
      }
      if (!mounted) return;
  ...
  Future<void> _resend() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _notice = 'A new verification email is on its way.');
      }
    } catch (_) {
      if (mounted) {
        setState(
          () => _notice =
              "We couldn't send another email yet. Please wait a few minutes and try again.",
        );
      }
    } finally {
  ...
  void _continue() => AppNavigation.replace(context, '/');

  @override
  Widget build(BuildContext context) {
    final verifying = _state == VerificationViewState.verifying;
    final success =
        _state == VerificationViewState.success ||
        _state == VerificationViewState.alreadyVerified;
            ...
            child: Card(
              child: Padding(
                ...
                child: Column(
                  children: [
                    Icon(
                      success ? Icons.verified : Icons.mark_email_read_outlined,
                      color: success
                          ? const Color(0xFF0C9F73)
                    ...
                    Text(
                      success
                          ? "Continue to the next step for your account."
                          : verifying
                          ? 'Please keep this page open while Firebase confirms your verification.'
