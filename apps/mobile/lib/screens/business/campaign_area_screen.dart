...
class CampaignAreaScreen extends StatefulWidget {
  ...
  @override
  State<CampaignAreaScreen> createState() =>
      _CampaignAreaScreenState();
}

class _CampaignAreaScreenState
    extends State<CampaignAreaScreen> {
  ...
  CampaignAreaShape _selectedShape =
      CampaignAreaShape.polygon;
  ...
  static const double _squareMetersPerAcre =
      4046.8564224;

  static const double _squareMetersPerSquareMile =
      2589988.110336;

  static const double _estimatedSweepSpacingMeters =
      30;

  static const double _walkingMetersPerMinute =
      75;

  static const int _productiveMinutesPerScaler =
      240;

  static const double _preliminaryHourlyRate =
      18;
  ...
  Future<void> _loadExistingArea() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Unable to load the existing zone area: $e',
          ),
        ),
      );
    }
  ...
  List<LatLng> _inputPointsForExistingShape(
    ...
    switch (shape) {
      case CampaignAreaShape.circle:
        final rawCenter =
            data['serviceAreaCenter'];

        final radius =
            data['serviceAreaRadiusMeters'];
        ...
        if (rawCenter is Map &&
          ...
          final center =
              LatLng(
            (rawCenter['latitude'] as num)
                .toDouble(),
            (rawCenter['longitude'] as num)
                .toDouble(),
          );
  ...
  List<LatLng> _buildCirclePolygon(
    LatLng center,
    LatLng edge,
  ) {
    final distance =
  ...
  bool _isAreaValid() {
    switch (_selectedShape) {
      ...
      case CampaignAreaShape.circle:
        ...
        if (_hasLoadedExistingArea &&
            _generatedArea.length >= 12) {
          return true;
        }

        return _generatedArea.length >= 12 &&
            radius != null &&
            radius > 0;
    }
  }
  ...
  ZoneMetrics? _calculateZoneMetrics() {
    ...
    final areaSquareMeters =
        _calculatePolygonAreaSquareMeters(
      _generatedArea,
    );

    final perimeterMeters =
        _calculatePerimeterMeters(
      _generatedArea,
    );

    final interiorTraversalMeters =
        areaSquareMeters /
            _estimatedSweepSpacingMeters;

    final estimatedWalkingMeters =
        perimeterMeters +
            interiorTraversalMeters;
    ...
    final estimatedMinutes =
      ...
      (
        estimatedWalkingMeters /
            _walkingMetersPerMinute
      ).ceil(),
    );

    final recommendedScalerCount =
        math.max(
      1,
      (
        estimatedMinutes /
            _productiveMinutesPerScaler
      ).ceil(),
    );
    ...
    final rawSuggestedPay =
        totalHours *
            _preliminaryHourlyRate;

    final suggestedBasePay =
        math.max(
      25,
      _roundToNearestFive(
        rawSuggestedPay,
    ...
    return ZoneMetrics(
      ...
      areaAcres:
          areaSquareMeters /
              _squareMetersPerAcre,
      areaSquareMiles:
          areaSquareMeters /
              _squareMetersPerSquareMile,
      perimeterMeters:
          perimeterMeters,
      perimeterMiles:
          perimeterMeters /
  ...
  double _calculatePolygonAreaSquareMeters(
    List<LatLng> points,
  ) {
    if (points.length < 3) {
      return 0;
    ...
    final centerLatitudeRadians =
        _degreesToRadians(
      center.latitude,
    );

    final centerLongitudeRadians =
        _degreesToRadians(
      center.longitude,
    );
    ...
    final projectedPoints =
        points.map(
      (point) {
        final pointLatitudeRadians =
            _degreesToRadians(
          point.latitude,
        );

        final pointLongitudeRadians =
            _degreesToRadians(
          point.longitude,
        );
  ...
  double _calculatePerimeterMeters(
    List<LatLng> points,
  ) {
    if (points.length < 2) {
      return 0;
  ...
  Future<void> _saveArea() async {
    ...
    final metrics =
        _calculateZoneMetrics();

    if (metrics == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Unable to calculate zone metrics.',
          ),
        ),
      );
    ...
    try {
      ...
      final updateData =
        ...
        'zoneAreaSquareMiles':
            metrics.areaSquareMiles,

        'zonePerimeterMeters':
            metrics.perimeterMeters,

        'zonePerimeterMiles':
            metrics.perimeterMiles,

        'estimatedWalkingMeters':
            metrics.estimatedWalkingMeters,

        'estimatedWalkingMiles':
            metrics.estimatedWalkingMiles,

        'estimatedMinutes':
            metrics.estimatedMinutes,

        'recommendedScalerCount':
            metrics.recommendedScalerCount,

        'suggestedBasePay':
            metrics.suggestedBasePay,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Unable to save campaign zone: $e',
          ),
        ),
      );
    } finally {
  ...
  CampaignAreaShape _shapeFromValue(
    String? value,
  ) {
    switch (value) {
      case 'rectangle':
