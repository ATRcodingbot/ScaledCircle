...
class CleanupCampaignScreen extends StatefulWidget {
  ...
  @override
  State<CleanupCampaignScreen> createState() => _CleanupCampaignScreenState();
}

class _CleanupCampaignScreenState extends State<CleanupCampaignScreen> {
  ...
  final scalerController = TextEditingController(text: "1");
  ...
  Future<void> pickCleanupLocation() async {
    final selectedLocation = await Navigator.push<CleanupLocationResult>(
      context,
  ...
  Future<void> createCleanupCampaign() async {
    ...
    try {
      final campaign = Campaign(
        ...
        type: CampaignType.yardCleanup,

        status: CampaignStatus.draft,

        materialSource: MaterialSource.businessProvided,

        workerBudget: double.tryParse(payController.text.trim()) ?? 0,

        platformFee: 0,

        totalFunding: double.tryParse(payController.text.trim()) ?? 0,

        bonus: double.tryParse(bonusController.text.trim()) ?? 0,

        targetQuantity: int.tryParse(scalerController.text.trim()) ?? 1,
      ...
      await _campaignService.createCampaign(campaign: campaign);

      if (!mounted) return;

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Cleanup campaign created!")),
      );
  ...
  Widget build(BuildContext context) {
    final pay = double.tryParse(payController.text.trim()) ?? 0;
        ...
        child: ListView(
          ...
          children: [
            ScCampaignHeader(
              ...
              description: "Create a verified cleanup job for Scalers.",

              locationText: addressController.text.isEmpty
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: "Cleanup Instructions",
            ...
            ScMapCard(
              ...
              onSelectLocation: pickCleanupLocation,
            ),
            ...
            ScPaymentCard(basePay: pay, bonus: bonus),

            ScCampaignSummaryCard(
              campaignName: nameController.text.isEmpty
                  ? "Unnamed Campaign"
                  : nameController.text,

              campaignType: "Job Site Cleanup",
              ...
              beforePhoto: requireBeforePhoto,

              afterPhoto: requireAfterPhoto,

              businessApproval: requireBusinessApproval,
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: createCleanupCampaign,

                icon: const Icon(Icons.rocket_launch),

                label: const Text("Launch Cleanup Campaign"),
              ),
            ),
          ],
        ),
