...
class _JobTrackingScreenState extends State<JobTrackingScreen> {
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      final zoneSnapshot =
          await widget.zone.reference.get();
      ...
      final simulated =
          routeSnapshot.data()?['simulated']
              == true;



      final tracking =
          routeSnapshot.data()?['tracking']
              == true;
  ...
  Future<void> _startTracking() async {
    ...
    try {
      ...
      if (status != 'in_progress') {
        ...
        throw Exception(
          'Start the job before GPS tracking.',
        );
...
List<LatLng> _buildSimulatedRoute() {
  ...
  try {
    ...
    if (simulatedRoute.length < 2) {
      ...
      throw Exception(
        'Could not generate walking route.',
      );
