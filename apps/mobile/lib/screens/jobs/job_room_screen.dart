...
import '../../widgets/job_room_completion_result.dart';
...
class _JobRoomScreenState extends State<JobRoomScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final completed =
        ['approved', 'completed', 'paid', 'settled'].contains(zone['status']) ||
        (completions.isNotEmpty &&
            completions.every(
              (c) =>
                  c['status'] == 'approved' || c['reviewStatus'] == 'approved',
            ));
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (!submitted &&
              !completed &&
              ['business', 'admin'].contains(data['viewerRole']))
          ...
          if (completions.isEmpty)
            ...
            ...completions.map((completion) {
              final status = completion['status']?.toString();
              final reviewStatus = completion['reviewStatus']?.toString();
              final scaler = jobRoomScalerName(data, completion);
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (evidence.isEmpty || !canvassing)
                        ExpansionTile(
                          title: const Text('Evidence Details'),
                          children: [Text(jobRoomEvidenceSummary(completion))],
                        ),
                      if (completion['submittedAt'] != null)
                        Text(
                          'Submitted: ${_formatDate(completion['submittedAt'])}',
                        ),
                      if (completion['reviewedAt'] != null)
                        Text(
                          'Reviewed: ${_formatDate(completion['reviewedAt'])}',
                        ),
          ...
          if (completed)
            const ExpansionTile(
              title: Text('Earlier coordination'),
              children: [
                ListTile(
                  title: Text(
                    'Materials and pre-start coordination are closed.',
                  ),
                  subtitle: Text(
                    'The completed work and review evidence remain available above.',
                  ),
                ),
              ],
            )
                  ...
                  FilledButton.icon(
                    onPressed: _acknowledgingReadiness
                        ...
                        : () async {
                            ...
                            try {
                              await _service.acknowledgeReadiness(
                                widget.zoneId,
                              );
                              await _load();
                              if (!mounted) return;
