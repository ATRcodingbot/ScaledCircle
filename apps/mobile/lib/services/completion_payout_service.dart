...
class CompletionPayoutService {
  ...
  Future<void> requestRedo({
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      if (status == 'paid') {
        throw Exception('A paid payout cannot be returned for redo.');
      }

      if (status != 'pending_review') {
        throw Exception(
          'This payout cannot be returned for redo from its current status.',
        );
      }
      ...
      if (zoneId == null || zoneId.isEmpty) {
        throw Exception('Payout record is missing the zone ID.');
      }

      DocumentReference<Map<String, dynamic>>? scalerWalletReference;

      DocumentSnapshot<Map<String, dynamic>>? walletSnapshot;

      if (scalerId != null && scalerId.isNotEmpty) {
        scalerWalletReference = _firestore.collection('wallets').doc(scalerId);

        walletSnapshot = await transaction.get(scalerWalletReference);
      }

      if (scalerWalletReference != null && walletSnapshot != null) {
      ...
      transaction.update(_firestore.collection('campaignZones').doc(zoneId), {
        'status': 'in_progress',
  ...
  Future<void> dropScaler({required String payoutId}) async {
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      if (status == 'paid') {
        throw Exception('A paid payout cannot have its Scaler dropped.');
      }
