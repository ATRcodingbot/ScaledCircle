...
function service(){const project=process.env.GCLOUD_PROJECT||process.env.GOOGLE_CLOUD_PROJECT,environment=process.env.APP_ENV;if(!validEnvironment(project,environment))throw new HttpsError('failed-precondition','Notifications are unavailable in this environment.');return createService({db:getFirestore(),auth:getAuth(),messaging:getMessaging(),FieldValue,Timestamp,project,environment});}
exports.mobileNotificationsV1=onCall(options,async request=>{try{const s=service(),{action,input={}}=request.data||{},uid=request.auth?.uid;if(action==='unregister')return await s.unregister(input);await s.actor(uid);if(action==='settings')return await s.settings(uid);if(action==='configure')return await s.configure(uid,input);if(action==='register')return await s.register(uid,input);if(action==='open')return await s.open(uid,input.notificationId);if(action==='check')return await s.check(uid,input);throw new HttpsError('invalid-argument','Choose a notification action.');}catch(e){if(e instanceof HttpsError)throw e;throw new HttpsError(['unauthenticated','permission-denied','invalid-argument','failed-precondition','resource-exhausted'].includes(e.code)?e.code:'unavailable','Notification request could not be completed. Please try again.');}});
...
exports.notifyBusinessEmailReplyV1=onDocumentCreated({...options,document:'businessMailboxes/{businessId}/replies/{replyId}',retry:true},async e=>{service();const reply=e.data.data();if(reply.businessId!==e.params.businessId)return;const op=(await getFirestore().doc(`businessMailboxes/${e.params.businessId}/operations/${reply.operationId}`).get()).data();if(op?.state!=='sent'||op.certification===true||op.businessId!==reply.businessId)return;return require('./signals').record({db:getFirestore(),FieldValue,kind:'reply',key:e.data.ref.path,after:reply});});
