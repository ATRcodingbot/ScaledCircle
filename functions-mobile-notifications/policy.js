...
const categories=['work','customers','growth','money','marketplace'];
const types={
 ...
 growth:['agent_daily_brief','agent_weekly_report','social_drafts_ready','social_post_published','email_campaign_review_ready','business_recommendations_ready'],
...
function policy(n){const category=Object.keys(types).find(c=>types[c].includes(n.type));if(!category)return null;
 return {category,immediate:category!=='growth',required:category==='account',title:{work:'Work & schedule update',customers:'Customer activity',growth:'Growth updates ready',money:'Earnings or account update',marketplace:'Work opportunity update',account:'ScaledCircle notification'}[category],body:n.type==='mobile_push_check'?'Your notification check is ready. Tap to open it.':'Open ScaledCircle to securely view the details.'};}
function preferences(value={}){return {enabled:value.enabled===true,categories:Object.fromEntries(categories.map(c=>[c,value.categories?.[c]!==false])),growthDigest:value.growthDigest!==false};}
